...
- **P0-3 UITest-Regression auf `testDeviceSmokeNavigationAndActions` — BEHOBEN am 2026-05-12 in Folge-Commit nach diesem Audit.** Root Cause: Heatmap-Button im `overviewRangeCard` war nur 13.3 pt hoch (HIG-Minimum 44 pt verletzt) und hatte keinen accessibilityIdentifier; im Phase-10-Hero-Map-Workspace war er außerhalb des XCUITest-`revealElement`-6-Swipe-Budgets, das zudem auf `firstMatch`-ScrollView swipt und in der neuen Hero-Map-/Hero-Filter-Layout ggf. den falschen ScrollView trifft. Fix: (1) UI — `.frame(minHeight: 44).contentShape(Rectangle()).accessibilityIdentifier("overview.range.heatmap.button")` an den Button; (2) UITest — Identifier-Lookup mit Fallback auf Label-Predicate, dazu neue `scrollUntilHittable(_:in:maxIterations:)`-Helper, der window-level Coordinate-Drag mit größerem Drag pro Iteration ausführt. **Verifikation:** `swift build` OK; `swift test` 1518/4/0 (116.5 s); `xcodebuild generic/platform=iOS` BUILD SUCCEEDED; signed Device-Build iPhone 15 Pro Max BUILD SUCCEEDED. Hardware-UITest-Suite **8/8 grün** auf iPhone 15 Pro Max (iOS 26.4): `testAppStoreScreenshots` 43.4 s, `testDeviceSmokeNavigationAndActions` 75.8 s, `testLandscapeLayoutSmoke` 597.4 s, `testLiveActivityHardwareCaptureDistance/Duration/Points/UploadStatusPendingAndRestart/UploadStatusFailed` 37–63 s.
