...
**Phase-Zeile 2026-05-08 (Phase 9A — Wrapper/AppFlow-Wiring + Settings-Delete-Button + Landing-View)**: LocalTimelineStore Phase 1..9A abgeschlossen. Wrapper (`wrapper/LH2GPXWrapper/ContentView.swift`) und Package-AppShell (`Sources/LocationHistoryConsumerApp/AppShellRootView.swift`) sind auf den Envelope-Pfad (`loadImportedFileEnvelope` + `LH2GPXAppFlow.apply(envelopeOutcome:to:preserveOnFailure:)`) verdrahtet; `LocalTimelineSessionLandingView` zeigt Session-Metadaten + Lösch-Button bei aktiver Store-Session; `AppTechnicalOptionsView` enthält Section "Local Timeline Store" mit Feature-Flag-Status + Lösch-Button. **Store-Pfad bleibt default AUS** (`LH2GPX_LOCAL_TIMELINE_STORE`). **KEIN Map/Heatmap/Overview UI-Hook**; **KEINE Darwin FileProtection-Aktivierung**; **KEINE Hardware-/AppStore-/TestFlight-Aussage**. **Diese Sektion bleibt FAILED / pending hardware retest** — die Phase-9A-Verdrahtung ändert das Hardware-Verhalten der Legacy-AppExport-Pipeline nicht.
