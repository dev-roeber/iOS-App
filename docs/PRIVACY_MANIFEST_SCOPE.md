...
> **Hinweis 2026-05-08 (Build-158-Vorbereitung — interne Test-Toggles, nur Bool)**: Build 157 ist Xcode Cloud grün und TestFlight-installierbar (Status „Überprüft", interne Tests erfolgreich); 46-MB-Gate bleibt **FAILED / pending hardware retest**. Da TestFlight-Tester keine Launch-Argumente/ENV setzen können, sind in `AppTechnicalOptionsView` zwei UserDefaults-basierte Toggles ergänzt (Sektion "Internal Test Toggles"), persistiert über `LocalTimelineTechnicalTestSettings` (`final class` ObservableObject, `.shared` + `init(userDefaults:)`). Keys (Namespace `LH2GPX.…`):
> - `LH2GPX.localTimelineStoreTestModeEnabled` — Bool, default `false`
> - `LH2GPX.importMemoryLoggingEnabled` — Bool, default `false`
>
> **Privacy-/Scope-Vertrag**: unter diesen Keys werden ausschließlich `Bool`-Werte gespeichert — **keine Standortdaten, keine Pfade, keine Tokens, keine Userdaten**. Pinpoint-Test: `testOnlyBoolsAreStoredUnderToggleKeys` (in `LocalTimelineTechnicalTestSettingsTests.swift`). Toggles aktivieren ausschließlich **interne Testmodi / Pre-production** (LocalTimelineStore-Feature-Flag und `ImportMemoryProbe`); Args/ENV bleiben primärer Aktivator, das Setting aktiviert zusätzlich, deaktiviert nichts. **Keine Änderung an `NSPrivacyCollectedDataTypes`**, keine neuen Persistenz-Pfade, keine neuen Netzwerkaktivitäten. Live-Upload, Recording und Auth-Flows unberührt. Footer-Hinweis am Gerät: "Internal/TestFlight only · Pre-production · Default off · No location data is stored in these settings". Store-Pfad bleibt default AUS. **Keine Privacy-Doku-Konsequenz für public-facing privacy.html**; Privacy-Doku-Update auf den tatsächlichen Rollout-Stand bleibt **Phase-10B/11-Pflicht** vor produktivem Default-Rollout.

> **Hinweis 2026-05-08 (Phase 10A — Store-DayMap UI Surface feature-flagged, kein MapKit-Import)**: Phase 10A führt **keine neuen Datentypen, keine neuen Persistenz-Pfade und keine neuen Netzwerkaktivitäten** ein. Die neue feature-flagged Store-DayMap-UI-Surface besteht aus einem Foundation-only Presentation Model (`LocalTimelineDayMapViewState`) und einem SwiftUI Placeholder (`LocalTimelineDayMapView`, `#if canImport(SwiftUI)`-guarded) **ohne MapKit-Import**; echte `MKMapView`-Verdrahtung bleibt **Phase-10B Mac/Xcode-Pflicht**. Candidate-Load liest ausschließlich path metadata über den bestehenden `StoreBackedMapDataProvider`/`LocalTimelineStoreReader` (**kein eager `coord_blob`-Decoding**); Geometrie wird ausschließlich für selektierte pathIDs lazy decodiert; harte `Budget`-Grenzen pro Route + pro Tag. Bounds-Fallback nutzt Visit-Koordinaten, die bereits in der Phase-2-Schemaerweiterung gespeichert werden (keine neuen Datenfelder). Store-Pfad bleibt default AUS (`LH2GPX_LOCAL_TIMELINE_STORE`). **Keine Privacy-Doku-Konsequenz für public-facing privacy.html**; Privacy-Doku-Update bleibt **Phase-10B/11-Pflicht** vor produktivem Default-Rollout.

> **Hinweis 2026-05-08 (Phase 9B — Store-DayList/DayDetail UI feature-flagged aktiv)**: Phase 9B führt **keine neuen Datentypen, keine neuen Persistenz-Pfade und keine neuen Netzwerkaktivitäten** ein. Die neue feature-flagged Store-DayList (`LocalTimelineDayListView`) und sheet-basierte DayDetail-UI (`LocalTimelineDayDetailView`) liest **bounded Metadaten** über den bestehenden `LocalTimelineStoreReader`/`LocalTimelineDayBrowserSource` (Day-Counts, Visits, Activities, Path-Metadaten); **kein eager `coord_blob`-Decoding, keine Karte, keine Polylines** — der DayDetail-Hinweis "Path points available (not decoded)" macht den fehlenden Decode explizit. Selection-State (`AppSessionState.selectedLocalTimelineDayId`) ist transient, **nicht persistiert in UserDefaults**. Store-Pfad bleibt default AUS (`LH2GPX_LOCAL_TIMELINE_STORE`). **Keine Privacy-Doku-Konsequenz für public-facing privacy.html**; Privacy-Doku-Update bleibt **Phase-10-Pflicht** vor produktivem Default-Rollout.
