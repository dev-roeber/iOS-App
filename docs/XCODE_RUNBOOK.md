...
## LocalTimelineStore (Phase 1..6-Spike eingecheckt, Linux-testbar, nicht UI-aktiv)

Stand 2026-05-08: **Phase 1..6-Spike eingecheckt**, isoliert, **nicht UI-aktiv** (Store bleibt pre-production). Auf Linux über die SQLite-C-API (`import SQLite3` via Apple-Plattformen bzw. `Sources/CSQLite/` Linux-Shim mit `pkg-config sqlite3`/`libsqlite3-dev`) gebaut und getestet. Phase 4 ergänzt Storage-Pfad-Resolver (`LocalTimelineStorageLocations` mit 4 Roots: DB unter `applicationSupportDirectory/LocationHistory2GPX/Imports/`, RenderCache unter `cachesDirectory`, ImportStaging + ExportStaging unter `temporaryDirectory`), Backup-Exclusion-Helper (`LocalTimelineFileAttributes`, Linux no-op), FileProtection-Kapselung (`LocalTimelineFileProtection`, Ziel `completeUnlessOpen`), Open-Lifecycle-Factory (`LocalTimelineStoreFactory`) und High-Level `deleteAllLocalTimelineData` über DB+WAL+SHM+RenderCache+ImportStaging+ExportStaging. Phase 5 ergänzt store-backed Streaming Export. **Phase 6** ergänzt feature-flagged AppSession-Quelle: `LocalTimelineFeatureFlags` (resolved `LH2GPX_LOCAL_TIMELINE_STORE` aus `ProcessInfo.arguments`/`environment`; default disabled; keine UserDefaults), `LocalTimelineSession` (Foundation-only Session-Modell), `LocalTimelineAppSessionAdapter` (bounded ViewState-Projektion), `LocalTimelineDeletionService` (Wrapper um `deleteAllLocalTimelineData`); Tests Linux-grün (4 neue Test-Dateien, 17 neue Cases). **Offene Darwin-Pflicht**: tatsächliche FileProtection-Aktivierung (`URLResourceKey.fileProtectionKey = .completeUnlessOpen` bzw. `SQLITE_OPEN_FILEPROTECTION_COMPLETEUNLESSOPEN` an `sqlite3_open_v2`) muss auf Apple-Plattformen aktiviert und in einem Darwin-Hardware-Pass verifiziert werden — Phase 4 hat den Hook nur dokumentiert; Phase 6 hat ihn nicht angefasst. **Keine neuen iOS-Build-Schritte** in diesem Runbook (**weiterhin kein UI-Hook**, kein AppContentLoader-Default auf Store, kein App-Session-Switch, keine automatische Migration; Store-Pfad ist gated by feature flag und niemals default-aktiv). Conditional-P0/P1-Gate (P0 falls 46-MB-Hardware-Retest FAILED, P1/P2 falls PASSED) ist in der Research-Doku dokumentiert.
