...
- [x] **GitHub Pages: Support + Index erstellt** — `docs/support.html` + `docs/index.html`; Support-URL: `https://dev-roeber.github.io/iOS-App/support.html`; Index-URL: `https://dev-roeber.github.io/iOS-App/` (2026-04-29)
...
- [x] **Xcode Cloud Release-Workflow gruen verifiziert** — Build 1.0 (44) erfolgreich via Xcode Cloud archiviert, auf TestFlight verfügbar und auf iPhone installierbar (2026-04-29)
- [x] **Overview-Map Freeze/Crash-Fix (Gesamtzeitraum + große Datenmengen)** — Hard `overlayLimit` in `OverviewMapRenderProfile` eingeführt (max 150–300 MapPolyline-Overlays je nach Datenmenge); `selectCandidates` capped auf Limit; `isOptimized=true` wenn Limit greift; Badge „Karte vereinfacht – Export vollständig" im View; Export unberührt; 647 Tests grün (2026-04-29)
- [x] **Overview-Map Performance-Audit: kein globales Coordinate-Budget nötig** — `overlayLimit × maxPolylinePoints` erzeugt implizites hartes Gesamtbudget (max 9.600–48.000 Koordinaten je Tier); 3 neue Invarianten-Tests; 650 Tests grün (2026-04-29)
- [x] **Interaktive Overview-/Explore-Karte gehärtet** — kein Re-Scan bei Pan/Zoom; Viewport-Selektion via Bounding-Box-Intersection statt nur Midpoint; stale Overlay-Tasks werden beim Neu-Load verworfen; Explore-Dismiss setzt wieder Full-View-Overlays; zusätzliche Model-/Viewport-Tests grün (2026-04-29)
- [ ] **Performance-Smoke-Test: Overview-Map mit echten/großen Datenmengen auf TestFlight verifizieren** — Fix auf TestFlight-Build deployen und mit realer Location-History (>20 MB, Gesamtzeitraum) auf echtem iPhone prüfen; kein Freeze/Crash erwartet; gelegentliche Ruckler in anderen Flows noch offen
- [x] **App-Store-Einreichung gestartet** — App Store Connect zeigt fuer `LH2GPX` Version `1.0` den Status `Warten auf Prüfung`; die Version ist damit eingereicht und App Review nicht mehr durch fehlenden Upload blockiert (repo-wahr dokumentiert 2026-04-30)
- [x] **Release-Signing-Konflikt im Repo entfernt** — explizite Release-`CODE_SIGN_IDENTITY` fuer App + Widget entfernt, `CODE_SIGN_STYLE = Automatic` bleibt aktiv; lokales `xcodebuild archive` fuer Build `45` laeuft wieder durch (2026-04-30)
- [ ] **ASC-Build-Zuordnung pruefen (52 vs. 55–57)** — auf der Versionsseite `1.0` ist aktuell Build `52` sichtbar, waehrend Xcode Cloud im Workflow `Release – Archive & TestFlight` erfolgreiche Builds `55`, `56` und `57` zeigt; klaeren, ob Build `52` bewusst eingereicht wurde oder ob ein neuerer Build (`57`) nachgereicht werden soll
- [ ] **Zertifikat auf ASCII-only Common Name ändern (Root Cause bewiesen)** — Build 34 vollständige IPA-Forensik: DR enthält CN in NFD (`6f cc 88` für ö), Zertifikat in NFC (`c3 b6`); Byte-Vergleich schlägt fehl; Xcode Cloud/macOS normalisiert Cert-CN zu NFD beim DR-Einbetten; Fix: (1) appleid.apple.com → Namen auf `Sebastian Roeber` ändern, (2) Distribution-Zertifikat in Xcode.app → Settings → Accounts revoken + neu erzeugen, (3) Xcode Cloud Clean Build
- [ ] **App ID + App Group im Developer Portal registrieren** — `de.roeber.LH2GPXWrapper` + `group.de.roeber.LH2GPXWrapper` (einmalig, manuell; nicht mehr wahrscheinlichste Build-34-Ursache, aber für automatisches Provisioning weiterhin erforderlich)
...
- [ ] Live Activity / Dynamic Island Hardware-Verifikation vervollstaendigen — offen bleiben Lock Screen, `minimal`, weitere Primärwerte und Fallback-Pfade; App Review dafuer weiterhin nicht repo-wahr freigegeben
...
- [x] Overview Map Spinner-Freeze-Fix — `!Task.isCancelled`-Guard in `loadMapData()` entfernt (Root Cause permanenter Spinner bei großem Import); `withTaskCancellationHandler` propagiert Outer-Cancellation in Detached-Task; Cancellation-Check vor DP-Phase + im Overlay-Loop; 636 Tests (2026-04-14)
...
- [ ] Live Activity / Dynamic Island auf echter Apple-Hardware weiter verifizieren: `iPhone 15 Pro Max` / `iOS 26.4` / `Debug`-Build bestaetigt bereits Recording-Start, Dynamic Island `compact` + `expanded` fuer Primärwert `Distanz` sowie Stop-/Dismiss-Verhalten; offen bleiben Lock Screen, `minimal`, Primärwert-Wechsel (`Dauer`, `Punkte`, `Upload-Status`) und Fallback-Hinweise bei deaktivierten / nicht verfuegbaren Live Activities
