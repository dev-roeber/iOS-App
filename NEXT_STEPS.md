...
- [ ] **App-Store-Einreichung** — aktuell kein lokaler Upload-Nachweis fuer Build `1.0 (45)`; nach Installation einer gueltigen Distribution-Identitaet bzw. via Xcode Cloud/App Store Connect den naechsten Build hochladen, unter Vertrieb auswaehlen und erst dann abhaken
...
- [ ] **Distribution-Zertifikat / ASC-Upload-Pfad lokal vervollstaendigen** — Host-Befund 2026-04-30: 0 Signing-Identities, 0 Provisioning-Profile, 0 ASC-API-Keys; `xcodebuild -exportArchive` scheitert mit `Failed to load profile` (kein Profil + kein Zertifikat); empfohlener Pfad: Xcode Cloud Workflow `Release – Archive & TestFlight` manuell starten (war 2026-04-29 einmalig gruen); alternativ Xcode Organizer → Distribute App → App Store Connect → Upload nach Zertifikat-Erstellung
