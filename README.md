...
- **Export**: GPX, KML, KMZ, GeoJSON, CSV; Filter nach Datum, Genauigkeit, Aktivitaetstyp, Bounding Box (TCX nur als Import-Format unterstützt)
...
Sources/
  ...
  Tests/LocationHistoryConsumerTests/    — Unit-Tests (aktueller Nachweis: 1057 Tests, 2 Skips, 0 Failures, 2026-05-07)
...
Aktueller Nachweis:
- `swift test` → `1057` Tests, `2` Skips, `0` Failures (Stand 2026-05-07, Phase 1-5 Audit-Train, HEAD `20877ae`: 14 Achsen über zwei Commits — `21b4026` (Phase 1: `projectedDays`-Cache, Mutations-Index, Race-Token, Live-Map-Dedup, `@testable`-Cleanup-Folge) + `20877ae` (Phase 2-5: Mock-Client + State-Transition-Tests, `LH2GPXAppFlow` Drift-Extraction + Auto-Restore-Phasen, API-Naming als additives Importing-Protokoll (kein Rename), `wrapper/CI.xctestplan` SwiftPM-Coverage als SKIP dokumentiert — pbxproj-Integration zu fragil, `Tests/README.md` Update, Doku-Truth-Cleanup). +1 Case gegenüber 1044 — Mock-Refactor ersetzt Placeholder durch zwei echte Cases, netto +1. Hardware-Re-Verifikation iPhone 15 Pro Max bleibt offen. Davor 1044 unter Audit-Batch B+C+D+A: Dead-Code-Removal in `AppDayDetailView`/`AppContentSplitView` und Löschung von `LHSharedMapChrome.swift` (~158 Zeilen weniger; `LHMapStyleToggleButton` public API entfernt — war seit MapLayerMenu-Train deprecated, keine externen Caller bekannt); Perf-Restposten (`OverviewMapRenderData: Equatable` Hand-`==`, inline Haversine, `HeatmapGridBuilder` Single-Sort+`suffix`-Trim, `AppExportQueries.findDay` Fast-Path); `@testable import` → reines `import` für 15 von 22 Test-Files; 9 neue Test-Files mit 27 neuen Cases (Decoder-Errors, GPX/TCX-Import-Errors, Round-Trip, Filter-Kombinationen, Heatmap-Edge-Cases, Live-State-Transition-Placeholder, Export-Mutations, ZIP-Streaming-Pfad). `wrapper/CI.xctestplan` SKIP (pbxproj-Integration out-of-scope), API-Naming P2-16 + HeatmapGridBuilder MapKit-Entkopplung P2-18 bewusst not done. Davor 1017 unter Audit Block 1-2-Train: WidgetSharedKeys-Konsolidierung mit `WidgetSharedKeys.swift` als Single-Source-of-Truth (P1-3 erledigt), `onOpenURL` im Package-App-Target (`AppShellRootView`, P1-4 erledigt); ZIP-Entry-Streaming für Google Timeline implementiert (Sniffer-basiert; greift bei genau einem Timeline-Entry, kein Mixed-ZIP — Peak RAM auf ~ein Element statt voller entpackter Datei); neuer Import-Phasen-Progress (`enum ImportPhase { reading, parsing, building }`, `LoadingProgressEngine.phase`, lokalisierte Phase-Labels im Wrapper-`ContentView`); Mikro-Benchmark als XCTest-`measure`-Baseline-Logging (kein fail-on-regression bar, kein gemessener Speedup-Faktor); 5 neue Tests gegenüber 1012. Davor: HEAD post-`70254ff` nach P0-Audit-Fix-Train 3/N: GPX-Crash-Härtung (`as!`/`fatalError` entfernt), KeychainHelper-Force-Unwrap entschärft, `AppExportSchemaVersion` ist jetzt forward-kompatibel — zukünftige Tool-Versionen sind decodierbar, `isSupportedByThisBuild` markiert unbekannte Schemas; LoadingBackground TimelineView 20 Hz; Streaming-Parser-Einführung — `GoogleTimelineStreamReader` plus `GoogleTimelineConverter.convertStreaming(contentsOf:)` für direkte JSON-Imports ohne Full-Data-Load; Performance-Pass auf der Pipeline am 2026-05-06: UnsafeBytes-Tokenizer (statt `Data.Index`), Default-Chunk 256 KB (vorher 64 KB), `autoreleasepool` um den Per-Element-Callback, Direct-Model-Build im Konverter (kein `[String: Any]`-Tree und kein JSON-Roundtrip mehr auf der Output-Seite — Konverter instanziiert `AppExport`/`Day`/`Visit`/`Activity`/`Path` direkt über neue public memberwise-Initializer); Test-Umfang unverändert; Auto-Restore lehnt rohe Google-Timeline-Dateien weiterhin grundsätzlich ab — unabhängig von der Größe (Sniffer-Skip) — zusätzlich zum bestehenden 50-MB-Cap; Mikro-Benchmark und Hardware-Re-Verifikation 46-MB-Datei auf iPhone 15 Pro Max bleiben offen — keine gemessenen Speedup-Faktoren, nur erwartete Größenordnungen / Designziel) WidgetSharedKeys-Konsolidierung mit `WidgetSharedKeys.swift` als Single-Source-of-Truth (P1-3 erledigt), `onOpenURL` im Package-App-Target (`AppShellRootView`, P1-4 erledigt); ZIP-Entry-Streaming für Google Timeline implementiert (Sniffer-basiert; greift bei genau einem Timeline-Entry, kein Mixed-ZIP — Peak RAM auf ~ein Element statt voller entpackter Datei); neuer Import-Phasen-Progress (`enum ImportPhase { reading, parsing, building }`, `LoadingProgressEngine.phase`, lokalisierte Phase-Labels im Wrapper-`ContentView`); Mikro-Benchmark als XCTest-`measure`-Baseline-Logging (kein fail-on-regression bar, kein gemessener Speedup-Faktor); 5 neue Tests gegenüber 1012. Davor: HEAD post-`70254ff` nach P0-Audit-Fix-Train 3/N: GPX-Crash-Härtung (`as!`/`fatalError` entfernt), KeychainHelper-Force-Unwrap entschärft, `AppExportSchemaVersion` ist jetzt forward-kompatibel — zukünftige Tool-Versionen sind decodierbar, `isSupportedByThisBuild` markiert unbekannte Schemas; LoadingBackground TimelineView 20 Hz; Streaming-Parser-Einführung — `GoogleTimelineStreamReader` plus `GoogleTimelineConverter.convertStreaming(contentsOf:)` für direkte JSON-Imports ohne Full-Data-Load; Performance-Pass auf der Pipeline am 2026-05-06: UnsafeBytes-Tokenizer (statt `Data.Index`), Default-Chunk 256 KB (vorher 64 KB), `autoreleasepool` um den Per-Element-Callback, Direct-Model-Build im Konverter (kein `[String: Any]`-Tree und kein JSON-Roundtrip mehr auf der Output-Seite — Konverter instanziiert `AppExport`/`Day`/`Visit`/`Activity`/`Path` direkt über neue public memberwise-Initializer); Test-Umfang unverändert; Auto-Restore lehnt rohe Google-Timeline-Dateien weiterhin grundsätzlich ab — unabhängig von der Größe (Sniffer-Skip) — zusätzlich zum bestehenden 50-MB-Cap; ZIP-Entry-Streaming, Mikro-Benchmark und Hardware-Re-Verifikation 46-MB-Datei auf iPhone 15 Pro Max bleiben offen — keine gemessenen Speedup-Faktoren, nur erwartete Größenordnungen / Designziel)
