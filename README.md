...
- eigentliche iOS-App / Produkt-UI fuer Tage, Einblicke, Heatmap, Live-Tracking, Upload-Konfiguration und lokale Exporte
- konsumiert den stabilen Contract aus dem Core-Repo, kann Google-Timeline-`location-history.json` und `.zip` aber lokal direkt importieren
- keine allgemeine Producer-Pipeline fuer Google-Rohdaten; Parsing-/Export-Kernlogik bleibt im Basis-Repo `LocationHistory2GPX`
- offline-first als Standardverhalten, ohne Pflicht-Cloud; optionaler nutzergesteuerter Live-Punkt-Upload ist vorhanden
- Apple-/Xcode-/Simulator-/Device-Endzustand ist auf diesem Linux-Host nicht frisch verifizierbar
...
Zusaetzlich gibt es jetzt die produktnahe App-Shell `LocationHistoryConsumerApp`. Sie startet leerer und import-zentriert, arbeitet standardmaessig offline-first und ist funktional bereits deutlich ausgebaut. Der optionale nutzergesteuerte Server-Upload akzeptierter Live-Recording-Punkte ist separat konfigurierbar und standardmaessig deaktiviert. Unter Linux ist nur der nicht-UI-Teil ueber `swift test` ehrlich verifizierbar; der Apple-/Store-/Device-Endzustand ist damit nicht frisch belegt.
...
Historisch belegt fuer einen Apple-Host am 2026-03-30:
...
Historisch belegt auf diesem Linux-Server fuer den Live-/Upload-/Insights-/Days-Batch:
...
Zusatz fuer diese konkrete Apple-Maschine aus dem historischen Nachweis: mit aktivem `/Library/Developer/CommandLineTools` schlug ein nacktes `swift test` an `no such module 'XCTest'` fehl. Die dokumentierten Apple-CLI-Laeufe wurden deshalb dort ehrlich mit `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer` gefahren.
