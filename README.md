...
Aktueller Nachweis:
- Linux (Stand 2026-05-08, Phase 9B `feat: wire local timeline day detail ui`): `swift test` Stand wird im nächsten Run-Sync nachgetragen. Phase 9B ergänzt feature-flagged **Store-DayList + DayDetail-UI** (`LocalTimelineDayListView`/`LocalTimelineDayDetailView`, beide `#if canImport(SwiftUI)`-guarded) über die bestehende `LocalTimelineSessionLandingView`. Bei gesetztem `LH2GPX_LOCAL_TIMELINE_STORE`-Flag zeigt der Store-Pfad nach Google-Timeline-Import jetzt eine Tagesliste (newest-first) und eine sheet-basierte Detail-Ansicht (Visits + Activities + Path-Metadaten, **kein eager `coord_blob`-Decoding, keine Map**); **Store-Pfad bleibt default AUS**. Davor Phase 9A mit 6 Linux-grünen Cases in `WrapperLocalTimelineEnvelopeRoutingTests.swift` auf den vorherigen Stand 1034/2/0.
