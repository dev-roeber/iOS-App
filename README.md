...
Aktueller Nachweis:
- **Mac (Stand 2026-05-12, HEAD `30015c9` — Post-Pull-Truth-Sync + Keychain-Hardening):** `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift test` → **1518 Tests, 4 Skips, 0 Failures** (105.9s). Die 2 zusätzlichen Skips ggü. 2 vorher sind die neuen Keychain-Accessibility-Read-Back-Tests, die auf macOS-File-Keychain das `pdmn`-Attribut nicht zurücklesen können — Save-Pfad selbst grün. `swift build` clean (112.6s). **`xcodebuild -scheme LH2GPXWrapper -destination 'generic/platform=iOS' build` BRICHT** im LH2GPXWidget-Linkschritt mit `Undefined symbols: _sqlite3_*` — pre-existing auf `origin/main` (`799adc5`), siehe `docs/DEEP_AUDIT_2026-05-12_POST_PULL.md` Sektion 9 P0-1. Mac-`swift test` umgeht das, weil dort SDK-`SQLite3` via `canImport(SQLite3)` greift; Xcode-iOS-Linker zieht aktuell stattdessen den Linux-CSQLite-Shim und scheitert.
