...
- **WeatherKit-Klassifizierung (implementiert):** `AppWeatherError.notProvisioned` + `isPermanent`-Flag + `AppWeatherDiagnostics.classify(...)`. WDSJWTAuth code=2 wird als permanenter Fehler erkannt; Live-Pfad deaktiviert Wetter-Layer und unterbindet 5-Min-Retry. Apple-Developer-Portal-Setup (WeatherKit-App-Service + Profil-Refresh) bleibt manueller Step ausserhalb Repo.
- **Train 3 Phase D-1 (implementiert, Map Controls + Context Clarity):** MapLayerMenu Tap-Region auf Apple-HIG-Floor 44 × 44 pt (visuelle Pill bleibt 34); `LHMapBase.LayerOrder` + `combinedBottomPillClearance` als zentrale Z-/Clearance-Tokens; CSV-Layer-Suppression-Hinweis im Export-Sheet; Insights-Status-Zeile mit aktivem Zeitraum + SurfaceMode; LocalTimelineImportProgressView lokalisiert (deutsche Phase-Strings); 5 Source-Contract-Tests.
- **Train 3 Phase D-0 (implementiert, Visual Readability Hardening):** Neue `mapGlass*Text`-Tokens fuer Map-backed Sheets; `ink/secondaryInk/tertiaryInk` dynamic via UIKit-Trait-Provider (Light = Navy, Dark = Cream); `LHGlassBottomSheetDashboard` erzwingt `\.colorScheme = .dark` auf Sheet-Content + staerkerer Base-Layer (0.28); Sheet-Header der acht migrierten Surfaces + `LHMapMetricCard` + `AppMapTabDashboardStrip` auf mapGlass-Tokens umgestellt. Welcome im Dark-Mode automatisch lesbar (ink ist jetzt dynamic).
- **Train 3 / Phase D-1 (geplant):** Recording-Dual-Truth, MapLayerMenu-Hit-Region 34→44, SurfaceMode/RangeFilter-Coupling, ActivityTimeline-Filter, CSV-Layer-Suppression, Performance-Profile scharf stellen, Instruments-Messplan.
...
- **Neu verifiziert:** Aktive Kartenflächen verwenden SwiftUI `Map`; kein produktiver UIKit-`MKMapView`-Renderer ist verdrahtet. UIKit-MapKit bleibt ein späterer Performance-Spike, nicht aktuelle App-Wahrheit.
- **Neu verifiziert:** Swift Charts wird in Insights/Speed/Elevation eingesetzt; Dateiimport/-export läuft über SwiftUI-Systemmodifikatoren; Insights-Sharing nutzt SwiftUI `ShareLink`.
- **Umgesetzt:** Low-Risk-Fix im Dateien-Tab zum Verwerfen lokaler Aktionsmeldungen.
- **Offen:** iPad-/regular-width-Smoke trotz `TARGETED_DEVICE_FAMILY = 1,2`, Light-Mode-/Force-Dark-Entscheidung, weitere Designsystem-Vereinheitlichung, Xcode/Device/TestFlight-Verifikation.
...
| Master · Phase E | FavoriteEntry Modell + lokale Persistenz/Migration | ✅ erledigt — `FavoriteEntry` + deterministic ID-Factory + JSON-Persistenz in Application Support + idempotente Legacy-Migration aus `DayFavoritesStore`; 12 Unit-Tests grün |
| Master · Phase F | echter CloudKit-Favoriten-Sync für `FavoriteEntry` | ⏳ folgt |
