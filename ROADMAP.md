...
- **Train F.7 Phase B-4 (implementiert):** `LGTabContainerView.mapTab` rendert `AppOverviewTracksMapView` full-bleed in `LHMapFirstPageScaffold`, Dashboard-Sheet hostet KPIs/Timeline/QuickActions. `LHMapFloatingChrome` bewusst nicht gemountet (Overlays im OverviewTracksMapView).
- **Train F.7 Phase B-5 (geplant):** Export-Preview.
- **Train F.7 Phase B (Rest):** Export-Preview → Heatmap → Editor → Explore-Sheet. Pro Screen ein PR.
