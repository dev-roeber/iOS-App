...
- **Deployment Target:** iOS 17.0 (App + Tests + `LH2GPXWidget`). Angehoben in Train F (2026-05-16, `chore: raise minimum ios target to 17`). Vorheriger Stand iOS 16.0/16.2 ist historisch. ActivityKit Live Activities (16.1) und Dynamic-Island-UI (16.2) sind jetzt im Minimum enthalten; die `@available(iOS 16.1/16.2, *)`-Gates im Source sind dadurch redundant, werden aber in Train F bewusst nicht abgebaut (Folge-Train).
